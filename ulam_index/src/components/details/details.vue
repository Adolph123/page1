              ...
              <div class="img"  style="background: #fff;width: 100%;transition: all 0.1s"  :style="{height:widths+'px'}">
                <swiper  v-if="iskeep"  :options="swiperOption" ref="mySwiper" >
                  ...
                  <swiper-slide v-for="(item,index) in bannerimages" :key="index">
                    <p  >
                      <img :src="item" alt="" class="imgswipe">
                    </p>
                  </swiper-slide>
...
<!--评论图片-->
                    <div style="display: flex;margin-top: 3px" v-if="item.images">
                      <img v-for="(todo,index) in item.images" :key="index" :src="todo" alt=""  @click.stop="taggleimagesdetail(todo)"  style="width:25%;display: block;height: auto;margin-right:7px;max-height: 100px">
                    </div>
                  </div>
                </div>
...
<!--      -->
        <div class="detailbtombox">
          ...
          <div class="detailbtomright">
            <span style="background: #9a55ff;display: flex;border-radius: 15px;overflow: hidden;width: 94%">
...
<script>
  ...
  import Lincoupon from './lincoupon/lincoupon';
  ...
  export default {
      ...
      created(){
          ...
          this.isskuid  = parseInt(this.$route.params.id);
      ...
      methods:{
          ...
          collectgoods(){
              ...
              let data= {id:parseInt(this.$route.params.id)};
              getdislist(apiurl,data).then(res => {
                  console.log(res,'数据');
                  this._getdislist(parseInt(this.$route.params.id));
              })
