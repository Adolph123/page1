<template>
  <div>
    <!--正常的显示-->
    <div>
      <div style="background-color: #3e3e3e;height:31px;margin-bottom: -1%; width: 100%; z-index: 999; position: fixed; top: 0px;"> </div><x-header  :left-options="{preventGoBack: true,backText: ''}" @on-click-back="fanhui1" style="position:fixed;top:30px;width:100%;z-index:999;"
      @on-click-title="ding">{{yous.name1}}的帖子</x-header>
      <div style="height:25px;"></div>
      <!--获取图片可用的宽度-->
      <flexbox :gutter="0" style="top:-1000px;">
      ...
      <!--循环流-->
      <div v-for="(todo,index1) in shuju" v-bind:key="todo.id">
        <div style="height:20px;"></div>
        <!--头像-->
        <flexbox :gutter="0" align="flex-start">
          <flexbox-item :span="1.7">
            <div style="text-align: center;">
              <img :src="todo.url.length>2 ?surl+'/img/header/'+ todo.url:'./static/tx.png'" alt=""
                style="width:75%;border-radius:4px;margin-top:5px;">
            </div>
          </flexbox-item>
          <flexbox-item :span="10">
            <div style="word-break:break-all;font-size:15px;">
              ...
              <!--<div style="margin-top:20px;" v-if="todo.picList">-->
                <!--<img class="previewer-demo-img" v-for="(item, index) in  todo.picList" :src="item.msrc" :style="ptk(todo.picH,todo.picW)" v-on:click="show(index,index1)">-->
                <!--<div v-transfer-dom>-->
              ...
              <div style="margin-top:15px;" v-show="todo.video">
                <div style="position:relative;">
                  <!--进入全屏-->
                  <img src="@/assets/icos/vbo.png" alt="" v-bind:class="[todo.isHorizontal ? 'bof1': 'bof2'] " v-on:click="palyda(index1,surl+todo.video,surl+todo.videoImg)">
                  <!--退出全屏-->
                  <div>
              ...
              <div style="margin-top:15px;">
                <flexbox :gutter="0">
                  ...
                  <flexbox-item :span="2">
                    <div style="">
                      <img :src="ltt" alt="" style="width:19px;position:relative;top:2.5px;" v-on:click="plun(todo,index1)">
    ...
    <actionsheet v-model="she3" :menus="{who:swt}" @on-click-menu="medi1" show-cancel></actionsheet>
  ...
  export default{
    ...
    data(){
      return {
        ...
        lredb:'./static/ltb.png',baox:'./static/1.png',
        list: [{
        ...
        //   查看某人的社区帖子
        swt:'',swt1:'',swtxian:false,yous:'',ios1:false,
        //视频窗  视频路劲
    ...
    mounted(){
      ...
      that.users = message
      //拿取路由值
      that.yous =  that.$route.params
      if(that.yous){
        that.token = that.yous.token
        that.nashuju()
      }else {
    ...
    watch:{
      ...
      qing(newb,oldb){
        if(newb){
          ...
          that.axios.get(that.surl+'/article?user='+that.users.hu+'&token='+that.token+'&page='+that.ppge+'&condition='+that.yous.name).then(response => {
    ...
    methods:{
        ...
        if(a == 'del'){
          ...
          that.axios.post(that.surl+'/delComment?token='+that.token,
            ...
            if(response.data.status == 'del_success'){
              ...
              that.shuju[that.acidd].comment = that.shuju[that.acidd].comment.filter(
                function(a){
      ...
      medi3(a){
        ...
        if(a == 'del'){
          ...
          that.axios.post(that.surl+'/delComment?token='+that.token,
            ...
            if(response.data.status == 'del_success'){
              //  同时要加dom元素进入，不需刷新显示数据
              that.shuju[that.acidd].comment[that.phfid].child = that.shuju[that.acidd].comment[that.phfid].child.filter(
                function(a){
                  return a.id != that.phfinfo.id
                }
              )
        ...
        if(that.iflun){
          ...
          that.axios.post(that.surl+'/comment?token='+that.token,
            ...
            if(response.data.status == 'comment_success'){
              //  同时要加dom元素进入，不需刷新显示数据
              that.shuju[that.acidd].comment[that.phfid].child.push(
                {
                  content:that.huifu,
                  name:that.users.hu,
          ...
          that.axios.post(that.surl+'/comment?token='+that.token,
            ...
            if(response.data.status == 'comment_success'){
              ...
              console.log('添加元素')
              //增加评论数
              that.shuju[that.pliid].commentCount = that.shuju[that.pliid].comment.length +1
              that.shuju[that.pliid].comment.unshift({
                child:[],
                reated: "1573459706",
                content:that.huifu,
                name:that.users.hu,
      ...
      showIcon(e){
        ...
        if(scrollTop + clientHeight + 20 >= scrHeight){
          that.qing = true
        }
        //如果超过多少距离显示返回顶部
        if(scrollTop > 800) {
      ...
      nashuju(){
        let that = this
        that.ss1 = true
        that.axios.get(that.surl+'/article?user='+that.users.hu+'&token='+that.token+'&condition='+that.yous.name).then(response => {
          that.ss1 = false
          console.log(response.data)
          if (response.data.status == "refresh"){
            that.token = response.data.code
